...
# pragma comment(link, "glew32.lib")
#endif
...
GLhandleARB makeProgramObjectFromFiles(
    ...
    const std::string & gFilename = "shader_g.glsl",
    const std::string & fFilename = "shader_f.glsl")
{
    ...
    {
        glslProgErrorExit(
            "ERROR - Could not read geometry shader code file: "
          + gFilename);
    }
    ...
    return makeProgramObject(vShaderSource, gShaderSource, fShaderSource);
}
                        ...
                        char ** argv=0)
{
    ...
    while (true)
    {
        ...
        std::cout << "`abc_v.glsl', `abc_g.glsl' and `abc_f.glsl'." << std::endl;
        std::cout << std::endl;
        std::cout << "===> ";  // Prompt
